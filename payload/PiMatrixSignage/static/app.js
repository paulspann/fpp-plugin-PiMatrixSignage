(() => {
  ...
  function setSetupSubtab(name,{persist=true,refresh=true}={}){
    ...
    if(name==='controller'){loadInterfaceMode();loadLicence();loadFppSetup();loadControllerUpdate(false);}
  }
  ...
  async function saveInterfaceMode(){
    ...
    $('saveInterfaceMode').disabled=true;try{const d=await api('/api/interface-mode',{method:'PUT',body:{mode}});renderInterfaceMode(d);toast(d.message||'Controller interface mode saved');}catch(e){toast(e.message,true);await loadInterfaceMode();}
  }
  ...
  function controllerHealthClass(f){return f?.ok?'ok':f?.can_apply===false?'error':'warn';}
  async function loadFppSetup(){
    try{
      ...
      $('controllerOutputMessage').textContent=f.message||'Panel controller status checked';$('controllerOutputMessage').classList.toggle('warn',!f.ok);$('controllerOutputBadge').textContent=f.ok?'Ready':f.can_apply===false?'Attention':'Apply required';$('controllerOutputBadge').className=`health-badge ${controllerHealthClass(f)}`;$('applyControllerOutput').disabled=f.can_apply===false;return f;
    }catch(e){$('fppSetup').innerHTML=`<p class="muted">${esc(e.message)}</p>`;$('controllerOutputMessage').textContent=e.message;$('controllerOutputMessage').classList.add('warn');$('controllerOutputBadge').textContent='Unavailable';$('controllerOutputBadge').className='health-badge error';throw e;}
  }
  async function applyControllerOutputConfig({confirmFirst=true,quiet=false}={}){
    ...
    $('applyControllerOutput').disabled=true;try{const r=await api('/api/controller-output/apply',{method:'POST',body:{}});await loadFppSetup();if(!quiet)toast(r.message||'Panel controller configuration applied');return r;}catch(e){await loadFppSetup().catch(()=>{});if(!quiet)toast(e.message,true);throw e;}finally{$('applyControllerOutput').disabled=false;}
  }
  ...
  function renderSoftwareUpdateNotice(data){
    const show=!!data?.available&&!!data?.platform_ready&&!controllerUpdateBusy(data),banner=$('softwareUpdateBanner'),badge=$('settingsUpdateBadge');
    if(banner){banner.classList.toggle('hidden',!show);if(show){const latest=String(data?.latest_version||'').trim(),current=String(data?.current_version||'').trim();$('softwareUpdateBannerText').textContent=latest?`Pi Matrix Signage v${latest} is ready to install${current?` · currently v${current}`:''}.`:`A newer Pi Matrix Signage release is ready to install${current?` · currently v${current}`:''}.`;}}
  ...
  function renderControllerUpdate(data){
    const st=data?.status||{},stateName=String(st.state||'').toLowerCase();$('controllerUpdateCurrent').textContent=`v${data?.current_version||'—'}`;$('controllerUpdateStatus').textContent=data?.available?(data?.latest_version?`v${data.latest_version} available`:'Update available'):data?.platform_ready?'Up to date':data?.checked_at?'Unable to check':'Waiting for background check';$('controllerUpdateInstallState').textContent=stateName?stateName.charAt(0).toUpperCase()+stateName.slice(1):'Ready';$('controllerUpdateChecked').textContent=formatUpdateChecked(data?.checked_at);$('controllerUpdateMessage').textContent=st.message||data?.message||'Controller software status';$('controllerUpdateMessage').classList.toggle('warn',!!data?.checked_at&&!data?.platform_ready||stateName==='failed');const installing=controllerUpdateBusy(data),allowed=can('users')&&!!data?.helper_ready&&!!data?.available&&!installing;$('installControllerUpdate').classList.toggle('hidden',!allowed);$('installControllerUpdate').disabled=!allowed;$('checkControllerUpdate').disabled=installing;renderSoftwareUpdateNotice(data);
  }
  ...
  $('detectedColorlightInterface').addEventListener('change',updateDetectedInterfaceWarning);
    ['panelWidth','panelHeight','panelsAcross','panelsDown'].forEach(id=>$(id).addEventListener('input',updateLayoutSummary));$('brightness').addEventListener('input',()=>{$('brightnessValue').textContent=`${$('brightness').value}%`;});$('saveSettings').addEventListener('click',saveSettings);$('activateLicence').addEventListener('click',activateLicence);$('checkLicence').addEventListener('click',checkLicence);$('clearLocalLicence').addEventListener('click',clearLocalLicence);$('openLicenseSetup').addEventListener('click',openLicenceSetup);$('saveInterfaceMode').addEventListener('click',saveInterfaceMode);$('openFppInterface').addEventListener('click',openFppInterface);$('refreshFpp').addEventListener('click',loadFppSetup);$('applyControllerOutput').addEventListener('click',()=>applyControllerOutputConfig());$('checkControllerUpdate').addEventListener('click',()=>loadControllerUpdate(true));$('installControllerUpdate').addEventListener('click',installControllerUpdate);$('openEngineeringAccess').addEventListener('click',openEngineeringAccess);$('saveGpioControls').addEventListener('click',saveGpioControls);$$('[data-gpio-field="action"]').forEach(x=>x.addEventListener('change',syncGpioEmergencyFields));$$('[data-gpio-test]').forEach(x=>x.addEventListener('click',()=>testGpioAction(x.dataset.gpioTest)));$('shutdownPi').addEventListener('click',shutdownPi);$('refreshDiagnostics').addEventListener('click',()=>loadDiagnostics(true));$('saveRecoverySettings').addEventListener('click',saveRecoverySettings);$('restartRenderer').addEventListener('click',()=>diagnosticsAction('restart-renderer','Restart the LED renderer now? The web interface stays online.'));$('restartFppd').addEventListener('click',()=>diagnosticsAction('restart-fppd','Restart the panel output service now? The LED output may pause briefly.'));$('clearRecoveryHistory').addEventListener('click',()=>diagnosticsAction('clear-history','Clear the recovery history?'));
