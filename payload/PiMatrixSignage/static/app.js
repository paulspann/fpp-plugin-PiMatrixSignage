(() => {
  ...
  function renderControllerUpdate(data){
    const st=data?.status||{},stateName=String(st.state||'').toLowerCase();$('controllerUpdateCurrent').textContent=`v${data?.current_version||'—'}`;$('controllerUpdateStatus').textContent=data?.available?'Update available':data?.platform_ready?'Up to date':'Unable to check';$('controllerUpdateInstallState').textContent=stateName?stateName.charAt(0).toUpperCase()+stateName.slice(1):'Ready';$('controllerUpdateMessage').textContent=st.message||data?.message||'Controller software status';$('controllerUpdateMessage').classList.toggle('warn',!data?.platform_ready||stateName==='failed');const installing=['queued','checking','installing','restarting'].includes(stateName),allowed=can('users')&&!!data?.helper_ready&&!!data?.available&&!installing;$('installControllerUpdate').classList.toggle('hidden',!allowed);$('installControllerUpdate').disabled=!allowed;$('checkControllerUpdate').disabled=installing;
  }
  ...
  async function refreshStatus(){try{const s=await api('/api/status');const p=$('statusPill');p.classList.toggle('ok',s.running&&!s.last_error&&!s.emergency);p.classList.toggle('error',!!s.last_error||!!s.emergency);p.querySelector('span:last-child').textContent=s.emergency?'EMERGENCY':s.last_error?'Output error':'Running';state.displayWidth=+s.width||0;state.displayHeight=+s.height||0;$('displaySize').textContent=`${s.width} × ${s.height} pixels`;$('framesSent').textContent=Number(s.frames_sent).toLocaleString();$('lastError').textContent=s.last_error||'None';$('activeSource').textContent=s.transition?`Exiting message #${s.transition.outgoing_message_id} · ${Number(s.transition.remaining||0).toFixed(1)}s`:s.emergency?`EMERGENCY · message #${s.emergency.id}`:s.manual?`Manual ${s.manual.type}${s.manual.id?' #'+s.manual.id:''}`:s.active?`${s.active.source} · ${s.active.type} #${s.active.id}`:'Blank';$('outputTarget').textContent=state.settings?`${state.settings.ddp_host}:${state.settings.ddp_port} · ${s.brightness?.effective??state.settings.brightness}%`:'DDP';renderNextSchedule(s.next_schedule);if($('emergencyPanel'))$('emergencyPanel').classList.toggle('active',!!s.emergency);$('activateEmergency').disabled=!!s.emergency||!state.settings?.emergency_message_id;$('clearEmergency').disabled=!s.emergency;if(can('schedules')&&$('page-schedules').classList.contains('active')&&Date.now()-(state._lastRuleRefresh||0)>4000){state._lastRuleRefresh=Date.now();api('/api/conditional-rules').then(v=>{state.conditionalRules=v;renderConditionalRuleList();if(state.selectedConditionalRule)selectConditionalRule(state.selectedConditionalRule);}).catch(()=>{});}}catch(e){const p=$('statusPill');p.classList.add('error');p.querySelector('span:last-child').textContent='Disconnected';}}
  ...
  $('openSchedulesFromDashboard').addEventListener('click',()=>document.querySelector('.tab[data-tab="schedules"]')?.click());
