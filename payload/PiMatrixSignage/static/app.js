(() => {
  ...
  function renderHardwareProfiles(){const list=$('hardwareProfileList'),profiles=state.hardwareProfiles||[];if(!profiles.length){list.innerHTML='<p class="muted">No hardware profiles saved yet.</p>';return;}list.innerHTML=profiles.map(p=>{const c=p.config||{};return `<div class="backup-row"><div class="backup-row-main"><strong>${esc(p.name)}</strong><small>${esc(c.panel_output_type==='colorlight'?`Colorlight ${(c.colorlight_receiver_model||'').toUpperCase()} · ${c.colorlight_interface}`:'Hanson rPI-MFC')} · ${c.panel_width}×${c.panel_height} ${esc(c.panel_scan)} · ${c.panels_across}×${c.panels_down} panels · ${c.brightness}% max</small></div><div class="backup-row-actions"><button class="btn secondary small" data-profile-apply="${p.id}">Apply</button><button class="btn danger ghost small" data-profile-delete="${p.id}">Delete</button></div></div>`;}).join('');$$('[data-profile-apply]').forEach(b=>b.addEventListener('click',()=>applyHardwareProfile(+b.dataset.profileApply)));$$('[data-profile-delete]').forEach(b=>b.addEventListener('click',()=>deleteHardwareProfile(+b.dataset.profileDelete)));}
