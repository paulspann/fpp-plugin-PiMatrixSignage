(() => {
  ...
  // Settings
  function renderHardwareDetection(){const d=state.settings?.hardware_detection||{},detected=!!d.rpi_mfc_detected,select=$('panelOutputType'),status=$('hardwareDetectionStatus');if(select){select.innerHTML=detected?'<option value="rpi_mfc">Hanson rPI-MFC</option><option value="colorlight">Colorlight receiver card</option>':'<option value="colorlight">Colorlight receiver card</option>';select.value=detected?(state.settings?.panel_output_type||'rpi_mfc'):'colorlight';if(![...select.options].some(o=>o.value===select.value))select.value=detected?'rpi_mfc':'colorlight';}if(status){if(detected){status.classList.remove('warn');status.innerHTML=`<strong>Hanson rPi-MFC detected:</strong> ${esc(d.cape_name||'Hanson rPi-MFC')}${d.cape_version?` · version ${esc(d.cape_version)}`:''}. ${esc(d.message||'FPP has confirmed the Hanson cape hardware.')} The Hanson direct-panel option is available.`;}else{status.classList.add('warn');status.innerHTML=d.source==='virtual_rpi_mfc_only'?'<strong>rPi-MFC configured but not physically confirmed:</strong> FPP is using an rPi-MFC virtual cape identity, but no physical Hanson EEPROM has been confirmed. The Hanson option is hidden and Colorlight is selected automatically; contact ISSL Support if a Hanson board is actually fitted.':'<strong>No Hanson rPi-MFC detected:</strong> the Hanson option has been hidden and Colorlight is selected automatically. Older rPi-MFC boards without a programmed FPP EEPROM may not be detectable; contact ISSL Support if this controller should have one.';}}}
  function updateOutputHardware(){const colorlight=$('panelOutputType').value==='colorlight';$('colorlightSettings').classList.toggle('hidden',!colorlight);$('colorlightWizardCard').classList.toggle('hidden',!colorlight);const intro=$('gpioControlsIntro'),help=$('gpioWiringHelp'),wiring={A:document.querySelector('[data-gpio-wiring="A"]'),B:document.querySelector('[data-gpio-wiring="B"]'),C:document.querySelector('[data-gpio-wiring="C"]')};if(colorlight){if(intro)intro.textContent='Use Raspberry Pi GPIO inputs for physical buttons or voltage-free relay contacts. These controls are wired to the Pi, not to the Colorlight receiver card.';if(help)help.innerHTML='<strong>Colorlight installations:</strong> connect dry/voltage-free contacts directly between the Raspberry Pi 40-pin header and GND: A = GPIO6 / physical pin 31, B = GPIO13 / physical pin 33, C = GPIO26 / physical pin 37. The software enables internal pull-ups. Do not connect a switch to the Colorlight receiver, and never apply 5V/12V or any external voltage to these GPIO inputs.';if(wiring.A)wiring.A.textContent='Pi GPIO6 · Header pin 31';if(wiring.B)wiring.B.textContent='Pi GPIO13 · Header pin 33';if(wiring.C)wiring.C.textContent='Pi GPIO26 · Header pin 37';}else{if(intro)intro.textContent='Use the three dedicated Hanson rPi-MFC user-input connectors for physical buttons or voltage-free relay contacts.';if(help)help.innerHTML='<strong>rPi-MFC inputs:</strong> A = CN2 / GPIO6 / header pin 31, B = CN3 / GPIO13 / header pin 33, C = CN4 / GPIO26 / header pin 37. The board is designed to use pull-ups, so use a voltage-free switch/relay contact to ground; never feed 5V/12V into a GPIO input.';if(wiring.A)wiring.A.textContent='CN2 · GPIO6 · Pin 31';if(wiring.B)wiring.B.textContent='CN3 · GPIO13 · Pin 33';if(wiring.C)wiring.C.textContent='CN4 · GPIO26 · Pin 37';}const allowed=colorlight?['1/4','1/8','1/16','1/32','1/64']:['1/8','1/16'];[...$('panelScan').options].forEach(o=>o.disabled=!allowed.includes(o.value));if(!allowed.includes($('panelScan').value))$('panelScan').value='1/16';$('panelHardwareWarning').innerHTML=colorlight?'<strong>Colorlight mapping:</strong> configure panel driver IC, scan rate and cabinet mapping on the receiver card first. Then create the matching ColorLight 5A-75 output in FPP.':'<strong>Physical panel order/orientation:</strong> set each panel wiring order and rotation once in FPP LED Panels. The rPI-MFC maximum panel count depends on panel size and scan rate.';}
