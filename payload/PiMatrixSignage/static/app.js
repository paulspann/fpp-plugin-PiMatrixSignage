(() => {
  ...
  function renderHardwareProfiles(){const list=$('hardwareProfileList'),profiles=state.hardwareProfiles||[];if(!profiles.length){list.innerHTML='<p class="muted">No hardware profiles saved yet.</p>';return;}const labels={rpi_mfc:'Hanson rPI-MFC',colorlight:'Colorlight receiver card',adafruit_hat:'Adafruit RGB Matrix HAT / Bonnet',adafruit_triple:'Adafruit Triple Matrix Bonnet'};list.innerHTML=profiles.map(p=>{const c=p.config||{},badge=p.builtin?'<span class="backup-pill">Built-in starter</span>':'',remove=p.builtin?'':`<button class="btn danger ghost small" data-profile-delete="${p.id}">Delete</button>`,hardware=c.panel_output_type==='colorlight'?`Colorlight ${(c.colorlight_receiver_model||'').toUpperCase()} · ${c.colorlight_interface}`:(labels[c.panel_output_type]||c.panel_output_type||'Unknown output');return `<div class="backup-row"><div class="backup-row-main"><div><strong>${esc(p.name)}</strong>${badge}</div><small>${esc(hardware)} · ${c.panel_width}×${c.panel_height} ${esc(c.panel_scan)} · ${c.panels_across}×${c.panels_down} panels · ${c.brightness}% max</small>${p.builtin?'<small class="muted">Starting point only — verify scan rate, driver mapping and colour order against the actual panel before commissioning.</small>':''}</div><div class="backup-row-actions"><button class="btn secondary small" data-profile-apply="${p.id}">Apply</button>${remove}</div></div>`;}).join('');$$('[data-profile-apply]').forEach(b=>b.addEventListener('click',()=>applyHardwareProfile(+b.dataset.profileApply)));$$('[data-profile-delete]').forEach(b=>b.addEventListener('click',()=>deleteHardwareProfile(+b.dataset.profileDelete)));}
