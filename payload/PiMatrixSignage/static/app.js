(() => {
  ...
  function updateSelectedFromControls(){
    ...
    if(l.type==='cloud-text'){l.cloud_text_items=$('cloudTextItems').value;l.cloud_interval=Math.max(.1,+$('cloudTextInterval').value||1.5);l.cloud_visible_for=Math.max(.2,+$('cloudTextVisibleFor').value||4);l.cloud_fade_in=Math.max(0,+$('cloudTextFadeIn').value||0);l.cloud_fade_out=Math.max(0,+$('cloudTextFadeOut').value||0);l.cloud_max_visible=clamp(Math.round(+$('cloudTextMaxVisible').value||3),1,12);l.cloud_gap=Math.max(0,+$('cloudTextGap').value||0);l.cloud_font=$('cloudTextFont').value;l.cloud_render_mode=$('cloudTextRenderMode').value;l.cloud_color_mode=$('cloudTextColorMode').value;l.color=$('cloudTextColor').value;l.cloud_palette=$('cloudTextPalette').value;}
