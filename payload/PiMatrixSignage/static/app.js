(() => {
  ...
  async function importPortableFile(file,expectedKind=''){
    if(!file)return null;if(expectedKind==='configuration'&&!confirm('Import this portable configuration?\n\nMessages, components, playlists, schedules, rules, brightness profiles and display settings will be replaced. User accounts and the underlying controller platform are left unchanged.'))return null;
  ...
  // background circles in the supplied Fledglings logo, plus black and white.
  ...
  function createColourPopover(){
    ...
    pop.querySelectorAll('[data-preset-colour]').forEach(b=>b.addEventListener('click',()=>{applyColourToActive(b.dataset.presetColour);refreshColourPopover();closeColourPopover();}));
  ...
  function applyPermissions(){
    ...
    if($('engineeringAccessCard'))$('engineeringAccessCard').classList.toggle('hidden',!(can('display_setup')&&can('users')));
  ...
  function setSetupSubtab(name,{persist=true,refresh=true}={}){
    ...
    if(name==='controller'){loadLicence();refreshControllerSection();}
  }
  ...
  async function loadAll() {
    try {
      ...
      if(can('display_setup')&&state.settings?.hardware_detection&&!state.settings.hardware_detection.rpi_mfc_detected&&state.settings.panel_output_type==='rpi_mfc'){state.settings=await api('/api/settings',{method:'PUT',body:{panel_output_type:'colorlight'}});}
      const jobs=[];
      ...
      if(can('display_setup')){populateSettings();await Promise.all([loadGpioControls(true),loadHardwareProfiles()]);}
  ...
  function defaultTextLayer(name='Text'){
    ...
    return Object.assign(l,{text:'YOUR MESSAGE',font:'',font_size:Math.max(8,Math.round(h*.7)),auto_fit:true,wrap:false,overflow:'manual',text_transform:'none',line_display:'together',typewriter_speed:12,flap_cycles:4,flap_stagger:.06,flap_order:'left',flap_board_style:'none',flap_bg_color:'#171717',flap_seam_color:'#000000',flap_border_color:'#454545',flap_cell_gap:1,flap_case_padding:2,nixie_build:false,nixie_build_duration:4,effect_amount:2,effect_stagger:.08,color:'#ffffff',color2:'#ff00ff',color_effect:'none',color_speed:1,color_palette:'#ff0000,#ffff00,#00ff00,#00aaff',glow:0,glow_color:'#ffffff',outline_color:'#000000',outline_width:0,padding:1,align:'center',valign:'middle',line_spacing:.12,shadow_color:'#000000',shadow_x:0,shadow_y:0,render_mode:'pixel',pixel_scale:1,pixel_bold:false,letter_spacing:0});
  }
  function defaultCloudTextLayer(){
    const {w,h}=logicalSize(); const l=baseAnimatedLayer('cloud-text','Cloud Text');
    return Object.assign(l,{w,h,cloud_text_items:'Message 1\nMessage 2\nText 3',cloud_interval:1.5,cloud_visible_for:4,cloud_fade_in:.6,cloud_fade_out:.8,cloud_max_visible:3,cloud_gap:2,cloud_font:'',cloud_render_mode:'pixel',cloud_color_mode:'solid',color:'#ffffff',cloud_palette:'#ffffff,#55ccff,#ffd45d,#ff78c8'});
  }
  ...
  function normalizeScene(sc){
    ...
    sc.layers.forEach((l,i)=>{
      ...
      if(['text','cloud-text','image','video','widget','icon','shader'].includes(l.type)){if(l.entrance_effect===undefined)l.entrance_effect='none';if(l.entrance_duration===undefined)l.entrance_duration=.5;if(l.exit_effect===undefined)l.exit_effect='none';if(l.exit_after===undefined)l.exit_after=0;if(l.exit_duration===undefined)l.exit_duration=.5;}
      if(l.type==='cloud-text'){if(l.cloud_text_items===undefined)l.cloud_text_items='Message 1\nMessage 2\nText 3';if(l.cloud_interval===undefined)l.cloud_interval=1.5;if(l.cloud_visible_for===undefined)l.cloud_visible_for=4;if(l.cloud_fade_in===undefined)l.cloud_fade_in=.6;if(l.cloud_fade_out===undefined)l.cloud_fade_out=.8;if(l.cloud_max_visible===undefined)l.cloud_max_visible=3;if(l.cloud_gap===undefined)l.cloud_gap=2;if(l.cloud_font===undefined)l.cloud_font='';if(!l.cloud_render_mode)l.cloud_render_mode='pixel';if(!l.cloud_color_mode)l.cloud_color_mode='solid';if(!l.color)l.color='#ffffff';if(!l.cloud_palette)l.cloud_palette='#ffffff,#55ccff,#ffd45d,#ff78c8';}
      ...
      if(['text','widget'].includes(l.type)){if(!l.render_mode)l.render_mode='pixel';if(!l.pixel_scale)l.pixel_scale=1;if(l.pixel_bold===undefined)l.pixel_bold=false;if(l.letter_spacing===undefined)l.letter_spacing=0;if(!l.overflow)l.overflow='manual';if(!l.text_transform)l.text_transform='none';if(!l.line_display)l.line_display='together';if(l.typewriter_speed===undefined)l.typewriter_speed=12;if(l.flap_cycles===undefined)l.flap_cycles=4;if(l.flap_stagger===undefined)l.flap_stagger=.06;if(!l.flap_order)l.flap_order='left';if(!l.flap_board_style)l.flap_board_style='none';if(!l.flap_bg_color)l.flap_bg_color='#171717';if(!l.flap_seam_color)l.flap_seam_color='#000000';if(!l.flap_border_color)l.flap_border_color='#454545';if(l.flap_cell_gap===undefined)l.flap_cell_gap=1;if(l.flap_case_padding===undefined)l.flap_case_padding=2;if(l.effect_amount===undefined)l.effect_amount=2;if(l.effect_stagger===undefined)l.effect_stagger=.08;if(!l.color_effect)l.color_effect='none';if(!l.color2)l.color2='#ff00ff';if(l.color_speed===undefined)l.color_speed=1;if(l.color_palette===undefined)l.color_palette='#ff0000,#ffff00,#00ff00,#00aaff';if(l.glow===undefined)l.glow=0;if(!l.glow_color)l.glow_color=l.color||'#ffffff';}
  ...
  function populateFonts(){
    for(const id of ['msgFont','layerFont','cloudTextFont','widgetFont']){const el=$(id);if(!el)continue;const current=el.value;el.innerHTML=option('','Default font')+state.fonts.map(f=>option(f.path,f.name)).join('');if([...el.options].some(o=>o.value===current))el.value=current;}
  }
  ...
  function layerLabel(l){
    ...
    if(l.type==='cloud-text')return String(l.cloud_text_items||'').split(/\r?\n/).filter(Boolean).slice(0,3).join(' · ');
  ...
  function updateNixieTextControls(){
    const animation=$('layerAnimation').value,nixie=animation==='nixie',segment=['seven-segment','fourteen-segment'].includes(animation),fixed=nixie||segment;
    ['layerFont','layerFontSize','layerAutoFit','layerWrap','layerRenderMode','layerPixelScale','layerPixelBold','layerLetterSpacing','layerTextTransform','layerLineDisplay'].forEach(id=>{const el=$(id);if(el)el.disabled=fixed;});
    const hint=$('nixieTextHint');if(hint)hint.classList.toggle('hidden',!nixie);
    const segmentHint=$('segmentTextHint');if(segmentHint)segmentHint.classList.toggle('hidden',!segment);
    const opts=$('nixieOptions');if(opts)opts.classList.toggle('hidden',!nixie);
    if(nixie&&$('layerText')){const cleaned=String($('layerText').value||'').replace(/\D/g,'').slice(0,8);if($('layerText').value!==cleaned)$('layerText').value=cleaned;}
  }
  function updateAnimationFieldVisibility(){
    const anim=$('layerAnimation').value,flap=anim==='split-flap',advanced=['glitch','character-wave'].includes(anim);
    $('splitFlapOptions').classList.toggle('hidden',!flap);$('advancedTextEffectOptions').classList.toggle('hidden',!advanced);updateNixieTextControls();
  }
  function updateLineSequenceTiming(){
    const el=$('lineSequenceTiming'),l=selectedLayer();if(!el||!l||l.type!=='text'){return;}
    const lines=String($('layerText')?.value||l.text||'').split(/\r?\n/).filter(x=>x.trim().length>0);
    const mode=$('layerLineDisplay')?.value||l.line_display||'together';
    if(mode!=='sequence'||lines.length<=1){el.textContent=lines.length>1?`${lines.length} lines display together`:'Single line';return;}
    const sceneTotal=sceneDuration(),delay=Math.max(0,+$('layerDelay')?.value||+l.delay||0),exitAfter=Math.max(0,+$('layerExitAfter')?.value||+l.exit_after||0);
    const available=Math.max(.001,Math.min(Math.max(0,sceneTotal-delay),exitAfter>0?exitAfter:Math.max(0,sceneTotal-delay)));
    const each=available/lines.length;el.textContent=`${lines.length} lines · ${each.toFixed(each<10?1:0)}s each`;
  }
  function loadSelectedLayerControls(){
    ...
    $('layerName').value=l.name||'';$('layerType').value=l.type||'text';populateLayerZoneSelect();$('layerZone').value=l.zone_id||'';$('layerEnabled').checked=l.enabled!==false;$('layerX').value=Math.round(+l.x||0);$('layerY').value=Math.round(+l.y||0);$('layerW').value=Math.max(1,Math.round(+l.w||1));$('layerH').value=Math.max(1,Math.round(+l.h||1));$('layerOpacity').value=l.opacity??100;$('layerRotation').value=l.rotation||0;$('layerDelay').value=l.delay||0;$('layerAnimation').value=l.animation||'static';$('layerSpeed').value=l.speed??30;$('layerEffectPeriod').value=l.effect_period??1;$('layerBlinkDuty').value=Math.round((l.blink_duty??.5)*100);$('layerFlapCycles').value=l.flap_cycles??4;$('layerFlapStagger').value=l.flap_stagger??.06;$('layerFlapOrder').value=l.flap_order||'left';$('layerFlapBoardStyle').value=l.flap_board_style||'none';$('layerFlapBgColor').value=l.flap_bg_color||'#171717';$('layerFlapSeamColor').value=l.flap_seam_color||'#000000';$('layerFlapBorderColor').value=l.flap_border_color||'#454545';$('layerFlapCellGap').value=l.flap_cell_gap??1;$('layerFlapCasePadding').value=l.flap_case_padding??2;$('layerNixieBuild').checked=!!l.nixie_build;$('layerNixieBuildDuration').value=l.nixie_build_duration??4;$('layerEffectAmount').value=l.effect_amount??2;$('layerEffectStagger').value=l.effect_stagger??.08;updateAnimationFieldVisibility();$('layerEntranceEffect').value=l.entrance_effect||'none';$('layerEntranceDuration').value=l.entrance_duration??.5;$('layerExitEffect').value=l.exit_effect||'none';$('layerExitAfter').value=l.exit_after??0;$('layerExitDuration').value=l.exit_duration??.5;$('layerTransitionProperties').classList.toggle('hidden',!['text','cloud-text','image','video','widget','icon','shader'].includes(l.type));
    $('textLayerProperties').classList.toggle('hidden',l.type!=='text');$('cloudTextLayerProperties').classList.toggle('hidden',l.type!=='cloud-text');$('imageLayerProperties').classList.toggle('hidden',l.type!=='image');$('videoLayerProperties').classList.toggle('hidden',l.type!=='video');$('shaderLayerProperties').classList.toggle('hidden',l.type!=='shader');$('widgetLayerProperties').classList.toggle('hidden',l.type!=='widget');$('iconLayerProperties').classList.toggle('hidden',l.type!=='icon');$('shapeLayerProperties').classList.toggle('hidden',l.type!=='shape');
    if(l.type==='text'){
      $('layerText').value=l.text||'';$('layerLineDisplay').value=l.line_display||'together';$('layerFont').value=l.font||'';$('layerFontSize').value=l.font_size||18;$('layerRenderMode').value=l.render_mode||'pixel';$('layerPixelScale').value=l.pixel_scale||1;$('layerPixelBold').checked=!!l.pixel_bold;$('layerLetterSpacing').value=l.letter_spacing||0;$('layerAutoFit').checked=!!l.auto_fit;$('layerWrap').checked=!!l.wrap;$('layerOverflow').value=l.overflow||'manual';$('layerTextTransform').value=l.text_transform||'none';$('layerTypewriterSpeed').value=l.typewriter_speed??12;$('layerColorEffect').value=l.color_effect||'none';$('layerColor2').value=l.color2||'#ff00ff';$('layerColorSpeed').value=l.color_speed??1;$('layerColorPalette').value=l.color_palette||'';$('layerGlow').value=l.glow||0;$('layerGlowColor').value=l.glow_color||l.color||'#ffffff';$('layerTextColor').value=l.color||'#ffffff';$('layerOutlineColor').value=l.outline_color||'#000000';$('layerOutlineWidth').value=l.outline_width||0;$('layerPadding').value=l.padding||0;$('layerAlign').value=l.align||'center';$('layerVAlign').value=l.valign||'middle';$('layerLineSpacing').value=l.line_spacing??.12;$('layerShadowColor').value=l.shadow_color||'#000000';$('layerShadowX').value=l.shadow_x||0;$('layerShadowY').value=l.shadow_y||0;updateLineSequenceTiming();updateNixieTextControls();
    }
    if(l.type==='cloud-text'){$('cloudTextItems').value=l.cloud_text_items||'';$('cloudTextInterval').value=l.cloud_interval??1.5;$('cloudTextVisibleFor').value=l.cloud_visible_for??4;$('cloudTextFadeIn').value=l.cloud_fade_in??.6;$('cloudTextFadeOut').value=l.cloud_fade_out??.8;$('cloudTextMaxVisible').value=l.cloud_max_visible??3;$('cloudTextGap').value=l.cloud_gap??2;$('cloudTextFont').value=l.cloud_font||'';$('cloudTextRenderMode').value=l.cloud_render_mode||'pixel';$('cloudTextColorMode').value=l.cloud_color_mode||'solid';$('cloudTextColor').value=l.color||'#ffffff';$('cloudTextPalette').value=l.cloud_palette||'';}
  ...
  function updateSelectedFromControls(){
    const l=selectedLayer();if(!l)return;beginHistoryBurst();l.name=$('layerName').value.trim()||l.type;l.zone_id=$('layerZone').value||'';l.enabled=$('layerEnabled').checked;l.x=+$('layerX').value||0;l.y=+$('layerY').value||0;l.w=Math.max(1,+$('layerW').value||1);l.h=Math.max(1,+$('layerH').value||1);l.opacity=clamp(+$('layerOpacity').value||0,0,100);l.rotation=+$('layerRotation').value||0;l.delay=Math.max(0,+$('layerDelay').value||0);l.animation=$('layerAnimation').value;l.speed=Math.max(0,+$('layerSpeed').value||0);l.effect_period=Math.max(.1,+$('layerEffectPeriod').value||1);l.blink_duty=clamp((+$('layerBlinkDuty').value||50)/100,.05,.95);l.flap_cycles=clamp(Math.round(+$('layerFlapCycles').value||4),1,12);l.flap_stagger=clamp(+$('layerFlapStagger').value||0,0,.5);l.flap_order=$('layerFlapOrder').value||'left';l.flap_board_style=$('layerFlapBoardStyle').value||'none';l.flap_bg_color=$('layerFlapBgColor').value;l.flap_seam_color=$('layerFlapSeamColor').value;l.flap_border_color=$('layerFlapBorderColor').value;l.flap_cell_gap=clamp(Math.round(+$('layerFlapCellGap').value||0),0,3);l.flap_case_padding=clamp(Math.round(+$('layerFlapCasePadding').value||2),1,6);l.nixie_build=$('layerNixieBuild').checked;l.nixie_build_duration=clamp(+$('layerNixieBuildDuration').value||4,.5,60);l.effect_amount=clamp(+$('layerEffectAmount').value||2,1,12);l.effect_stagger=clamp(+$('layerEffectStagger').value||.08,0,.5);updateAnimationFieldVisibility();
    if(['text','cloud-text','image','video','widget','icon','shader'].includes(l.type)){l.entrance_effect=$('layerEntranceEffect').value;l.entrance_duration=Math.max(.05,+$('layerEntranceDuration').value||.5);l.exit_effect=$('layerExitEffect').value;l.exit_after=Math.max(0,+$('layerExitAfter').value||0);l.exit_duration=Math.max(.05,+$('layerExitDuration').value||.5);}
    if(l.type==='text'){if(l.animation==='nixie'){$('layerText').value=String($('layerText').value||'').replace(/\D/g,'').slice(0,8);}l.text=$('layerText').value;l.line_display=['nixie','seven-segment','fourteen-segment'].includes(l.animation)?'together':($('layerLineDisplay').value||'together');l.font=$('layerFont').value;l.font_size=Math.max(4,+$('layerFontSize').value||18);l.render_mode=$('layerRenderMode').value;l.pixel_scale=Math.max(1,Math.min(8,+$('layerPixelScale').value||1));l.pixel_bold=$('layerPixelBold').checked;l.letter_spacing=Math.max(0,Math.min(8,+$('layerLetterSpacing').value||0));l.auto_fit=$('layerAutoFit').checked;l.wrap=$('layerWrap').checked;l.overflow=$('layerOverflow').value;l.text_transform=$('layerTextTransform').value;l.typewriter_speed=Math.max(.1,+$('layerTypewriterSpeed').value||12);l.color_effect=$('layerColorEffect').value;l.color2=$('layerColor2').value;l.color_speed=Math.max(.05,+$('layerColorSpeed').value||1);l.color_palette=$('layerColorPalette').value;l.glow=Math.max(0,+$('layerGlow').value||0);l.glow_color=$('layerGlowColor').value;l.color=$('layerTextColor').value;l.outline_color=$('layerOutlineColor').value;l.outline_width=Math.max(0,+$('layerOutlineWidth').value||0);l.padding=Math.max(0,+$('layerPadding').value||0);l.align=$('layerAlign').value;l.valign=$('layerVAlign').value;l.line_spacing=clamp(+$('layerLineSpacing').value||0,0,1);l.shadow_color=$('layerShadowColor').value;l.shadow_x=+$('layerShadowX').value||0;l.shadow_y=+$('layerShadowY').value||0;updateLineSequenceTiming();}
    if(l.type==='cloud-text'){l.cloud_text_items=$('cloudTextItems').value;l.cloud_interval=Math.max(.1,+$('cloudTextInterval').value||1.5);l.cloud_visible_for=Math.max(.5,+$('cloudTextVisibleFor').value||4);l.cloud_fade_in=Math.max(.2,+$('cloudTextFadeIn').value||.2);l.cloud_fade_out=Math.max(.2,+$('cloudTextFadeOut').value||.2);l.cloud_max_visible=clamp(Math.round(+$('cloudTextMaxVisible').value||3),1,12);l.cloud_gap=Math.max(0,+$('cloudTextGap').value||0);l.cloud_font=$('cloudTextFont').value;l.cloud_render_mode=$('cloudTextRenderMode').value;l.cloud_color_mode=$('cloudTextColorMode').value;l.color=$('cloudTextColor').value;l.cloud_palette=$('cloudTextPalette').value;}
  ...
  function addLayer(type){
    if(!state.scene)state.scene=templateScene('blank');recordHistory();let l;if(type==='cloud-text')l=defaultCloudTextLayer();else if(type==='image')l=defaultImageLayer();else if(type==='video')l=defaultVideoLayer();else if(type==='shader')l=defaultShaderLayer();else if(type==='widget')l=defaultWidgetLayer();else if(type==='icon')l=defaultIconLayer();else if(type==='shape')l=defaultShapeLayer();else l=defaultTextLayer();const maxZ=state.scene.layers.reduce((m,x)=>Math.max(m,+x.z||0),0);l.z=maxZ+10;state.scene.layers.push(l);setSelection([l.id],l.id);renderLayerList();loadSelectedLayerControls();scheduleEditorPreview();
  }
  ...
  function renderBackupStatus(status={}){
    ...
    const labels={ready:'Ready',queued:'Queued',creating:'Creating backup…',safety_backup:'Creating safety backup…',restoring:'Restoring Pi Matrix Signage…',restoring_fpp:'Restoring controller platform…',success:'Complete',failed:'Failed'};
  ...
  async function createBackup(){
    ...
    try{await api('/api/backups/create',{method:'POST',body:{}});renderBackupStatus({state:'queued',operation:'backup',message:'Backup requested; collecting Pi Matrix Signage data and controller-platform configuration…'});scheduleBackupPoll();toast('Backup started');}
    catch(e){toast(e.message,true);}
  }
  ...
  async function restoreExistingBackup(filename){
    if(state.backupBusy)return;
    const network=$('backupKeepNetwork').checked?'Current controller network settings will be kept.':'WARNING: controller network settings WILL be restored and this Pi may change IP/network.';
  ...
  function renderHardwareDetection(){
    const d=state.settings?.hardware_detection||{},detected=!!d.rpi_mfc_detected,select=$('panelOutputType'),status=$('hardwareDetectionStatus');
    const manual='<option value="colorlight">Colorlight receiver card</option><option value="adafruit_hat">Adafruit RGB Matrix HAT / Bonnet</option><option value="adafruit_triple">Adafruit Triple Matrix Bonnet</option>';
    if(select){
      select.innerHTML=(detected?'<option value="rpi_mfc">Hanson rPI-MFC</option>':'')+manual;
      const wanted=state.settings?.panel_output_type||(detected?'rpi_mfc':'colorlight');
      select.value=[...select.options].some(o=>o.value===wanted)?wanted:'colorlight';
    }
    if(status){
      if(detected){
        status.classList.remove('warn');
        status.innerHTML=`<strong>Hanson rPi-MFC detected:</strong> ${esc(d.cape_name||'Hanson rPi-MFC')}${d.cape_version?` · version ${esc(d.cape_version)}`:''}. ${esc(d.message||'The controller platform has confirmed the Hanson cape hardware.')} Hanson, Colorlight and the two Adafruit adapter choices are available.`;
      }else{
        status.classList.add('warn');
        status.innerHTML=d.source==='virtual_rpi_mfc_only'
          ?'<strong>rPi-MFC configured but not physically confirmed:</strong> the controller platform has an rPi-MFC virtual identity, but no physical Hanson EEPROM was confirmed. Hanson is hidden. Colorlight and Adafruit adapters remain available; Adafruit boards do not provide a reliable common auto-detection signal and must be selected manually.'
          :'<strong>No Hanson rPi-MFC detected:</strong> Hanson is hidden. Colorlight and Adafruit RGB Matrix adapters remain available. Adafruit HAT/Bonnet hardware cannot be reliably auto-detected across all revisions, so select the fitted Adafruit board manually.';
      }
    }
  }

  function updateOutputHardware(){
    const type=$('panelOutputType').value;
    const colorlight=type==='colorlight', adafruitHat=type==='adafruit_hat', adafruitTriple=type==='adafruit_triple', adafruit=adafruitHat||adafruitTriple;
    $('colorlightSettings').classList.toggle('hidden',!colorlight);
    $('colorlightWizardCard').classList.toggle('hidden',!colorlight);
    $('adafruitSettings')?.classList.toggle('hidden',!adafruit);
    const adafruitHelp=$('adafruitOutputHelp');
    if(adafruitHelp&&adafruitHat){
      adafruitHelp.innerHTML='<strong>Direct-HUB75 setup:</strong> Pi Matrix Signage will configure the controller for the Adafruit <code>adafruit-hat</code> mapping with one parallel output. Do not use the PWM hardware modification unless the documented GPIO4→GPIO18 change is physically present. This direct-output driver is for Raspberry Pi 4-class controllers, not Pi 5.';
    }else if(adafruitHelp&&adafruitTriple){
      adafruitHelp.innerHTML='<strong>Active3 setup:</strong> Pi Matrix Signage will configure the controller for the standard mapping with <strong>3 parallel HUB75 outputs</strong>. Each IDC socket is a separate parallel string. Provide panel power separately; the Triple Matrix Bonnet does not power the LED panels. This direct-output driver is for Raspberry Pi 4-class controllers, not Pi 5.';
    }

    const intro=$('gpioControlsIntro'),help=$('gpioWiringHelp'),toggle=$('gpioControlsEnabled'),card=$('gpioControlsCard');
    const wiring={A:document.querySelector('[data-gpio-wiring="A"]'),B:document.querySelector('[data-gpio-wiring="B"]'),C:document.querySelector('[data-gpio-wiring="C"]')};
    if(adafruit){
      if(intro)intro.textContent='Physical GPIO controls are unavailable while an Adafruit direct-HUB75 adapter is driving the display.';
      if(help)help.innerHTML='<strong>GPIO conflict:</strong> the Adafruit RGB Matrix mappings use GPIO6, GPIO13 and GPIO26 as HUB75 panel signals. Pi Matrix Signage will not start button/relay monitoring on those lines while this output is selected. Use the web remote, schedules or another external control method instead.';
      if(wiring.A)wiring.A.textContent='Reserved by HUB75 output';if(wiring.B)wiring.B.textContent='Reserved by HUB75 output';if(wiring.C)wiring.C.textContent='Reserved by HUB75 output';
    }else if(colorlight){
      if(intro)intro.textContent='Use Raspberry Pi GPIO inputs for physical buttons or voltage-free relay contacts. These controls are wired to the Pi, not to the Colorlight receiver card.';
      if(help)help.innerHTML='<strong>Colorlight installations:</strong> connect dry/voltage-free contacts directly between the Raspberry Pi 40-pin header and GND: A = GPIO6 / physical pin 31, B = GPIO13 / physical pin 33, C = GPIO26 / physical pin 37. The software enables internal pull-ups. Do not connect a switch to the Colorlight receiver, and never apply 5V/12V or any external voltage to these GPIO inputs.';
      if(wiring.A)wiring.A.textContent='Pi GPIO6 · Header pin 31';if(wiring.B)wiring.B.textContent='Pi GPIO13 · Header pin 33';if(wiring.C)wiring.C.textContent='Pi GPIO26 · Header pin 37';
    }else{
      if(intro)intro.textContent='Use the three dedicated Hanson rPi-MFC user-input connectors for physical buttons or voltage-free relay contacts.';
      if(help)help.innerHTML='<strong>rPi-MFC inputs:</strong> A = CN2 / GPIO6 / header pin 31, B = CN3 / GPIO13 / header pin 33, C = CN4 / GPIO26 / header pin 37. The board is designed to use pull-ups, so use a voltage-free switch/relay contact to ground; never feed 5V/12V into a GPIO input.';
      if(wiring.A)wiring.A.textContent='CN2 · GPIO6 · Pin 31';if(wiring.B)wiring.B.textContent='CN3 · GPIO13 · Pin 33';if(wiring.C)wiring.C.textContent='CN4 · GPIO26 · Pin 37';
    }
    if(card){for(const el of card.querySelectorAll('input,select,button'))el.disabled=adafruit;}
    if(toggle&&adafruit)toggle.checked=false;

    const allowed=colorlight?['1/4','1/8','1/16','1/32','1/64']:adafruit?['1/4','1/8','1/16','1/32']:['1/8','1/16'];
    [...$('panelScan').options].forEach(o=>o.disabled=!allowed.includes(o.value));
    if(!allowed.includes($('panelScan').value))$('panelScan').value='1/16';
    if(colorlight){
      $('panelHardwareWarning').innerHTML='<strong>Colorlight mapping:</strong> configure the panel driver IC, scan rate and cabinet mapping on the receiver card first. Save these Display settings and Pi Matrix Signage will configure the matching panel output automatically.';
    }else if(adafruitHat){
      $('panelHardwareWarning').innerHTML='<strong>Adafruit HAT / Bonnet:</strong> Pi Matrix Signage configures the Adafruit HUB75 pinout with one parallel output. Verify panel scan/address mapping and any Address-E jumper required by 64×64 panels before commissioning.';
    }else if(adafruitTriple){
      $('panelHardwareWarning').innerHTML='<strong>Adafruit Triple / Active3:</strong> Pi Matrix Signage configures three parallel HUB75 outputs. Connect each physical string to the intended output, verify the logical layout and power the panels independently.';
    }else{
      $('panelHardwareWarning').innerHTML='<strong>Physical panel order/orientation:</strong> Pi Matrix Signage preserves a previously commissioned panel mapping where possible and manages the panel output underneath. Use the panel tests to confirm order and rotation after any hardware change. The rPi-MFC maximum panel count depends on panel size and scan rate.';
    }
  }
  function populateSettings(){const s=state.settings;if(!s)return;$('panelModel').value=s.panel_model||'Custom P5/P10 panel';$('panelWidth').value=s.panel_width;$('panelHeight').value=s.panel_height;$('panelScan').value=s.panel_scan||'1/16';$('panelsAcross').value=s.panels_across;$('panelsDown').value=s.panels_down;$('displayRotation').value=s.display_rotation;renderHardwareDetection();$('colorlightReceiverModel').value=s.colorlight_receiver_model||'5a-75b';$('colorlightInterface').value=s.colorlight_interface||'eth1';$('brightness').value=s.brightness;$('brightnessValue').textContent=`${s.brightness}%`;$('frameRate').value=s.frame_rate;$('colorOrder').value=s.color_order;$('timezone').value=s.timezone;$('ddpHost').value=s.ddp_host;$('ddpPort').value=s.ddp_port;$('ddpOffset').value=s.ddp_offset;$('defaultMessage').value=s.default_message_id||'';updateOutputHardware();updateLayoutSummary();renderCommissioning();}
  ...
  async function saveSettings(){try{const body={panel_model:$('panelModel').value.trim(),panel_width:+$('panelWidth').value,panel_height:+$('panelHeight').value,panel_scan:$('panelScan').value,panels_across:+$('panelsAcross').value,panels_down:+$('panelsDown').value,display_rotation:+$('displayRotation').value,panel_output_type:$('panelOutputType').value,colorlight_receiver_model:$('colorlightReceiverModel').value,colorlight_interface:$('colorlightInterface').value.trim(),brightness:+$('brightness').value,frame_rate:+$('frameRate').value,color_order:$('colorOrder').value,timezone:$('timezone').value.trim(),ddp_host:$('ddpHost').value.trim(),ddp_port:+$('ddpPort').value,ddp_offset:+$('ddpOffset').value,default_message_id:$('defaultMessage').value?+$('defaultMessage').value:null};state.settings=await api('/api/settings',{method:'PUT',body});populateSettings();let applied=false;try{await applyControllerOutputConfig({confirmFirst:false,quiet:true});applied=true;}catch(_){await loadFppSetup();}await loadControllerHealth().catch(()=>{});toast(applied?'Display settings saved and panel controller updated':'Display settings saved; panel controller needs attention',!applied);}catch(e){toast(e.message,true);}}
  function renderInterfaceMode(data){
    const mode=data?.mode==='appliance'?'appliance':'fpp',fpp=$('interfaceModeFpp'),appliance=$('interfaceModeAppliance');if(fpp)fpp.checked=mode==='fpp';if(appliance)appliance.checked=mode==='appliance';
    const badge=$('interfaceModeBadge'),msg=$('interfaceModeMessage'),save=$('saveInterfaceMode');if(badge){badge.textContent=mode==='appliance'?'Appliance mode':'FPP-first';badge.className=`health-badge ${data?.in_sync===false?'warn':'ok'}`;}if(msg)msg.textContent=data?.message||'';if(save){save.disabled=!can('users')||!data?.helper_ready;save.title=!can('users')?'Users permission is required to change the controller interface mode':(!data?.helper_ready?'Install this release once before changing interface mode':'');}
  }
  async function loadInterfaceMode(){try{const d=await api('/api/interface-mode');renderInterfaceMode(d);return d;}catch(e){$('interfaceModeBadge').textContent='Unavailable';$('interfaceModeBadge').className='health-badge error';$('interfaceModeMessage').textContent=e.message;$('saveInterfaceMode').disabled=true;return null;}}
  async function saveInterfaceMode(){
    const mode=document.querySelector('input[name="controllerInterfaceMode"]:checked')?.value;if(!mode){toast('Choose a controller interface mode',true);return;}const description=mode==='appliance'?'make Pi Matrix Signage the controller home page':'restore FPP as the controller home page';if(!confirm(`Save this interface mode and ${description}? No reinstall is required.`))return;
    $('saveInterfaceMode').disabled=true;try{const d=await api('/api/interface-mode',{method:'PUT',body:{mode}});renderInterfaceMode(d);await loadControllerHealth().catch(()=>{});toast(d.message||'Controller interface mode saved');}catch(e){toast(e.message,true);await loadInterfaceMode();}
  }
  function openFppInterface(){const host=location.hostname.includes(':')?`[${location.hostname}]`:location.hostname;window.open(`${location.protocol}//${host}/index.php`,'_blank','noopener');}

  function controllerOutputClass(f){return f?.ok?'ok':f?.can_apply===false?'error':'warn';}
  async function loadFppSetup(){
    try{
      const f=await api('/api/controller-output'),nic=f.network_interface?`<div class="fpp-value"><span>Dedicated interface</span><strong>${esc(f.network_interface)} · ${f.interface_present?'detected':'not detected'}</strong></div>`:'';
      $('fppSetup').innerHTML=`<div class="fpp-value"><span>Output hardware</span><strong>${esc(f.output_label)}</strong></div>${nic}<div class="fpp-value"><span>Panel arrangement</span><strong>${f.panels_across} across × ${f.panels_down} down</strong></div><div class="fpp-value"><span>Panel pixel size</span><strong>${esc(f.panel_size)} · ${esc(f.panel_scan)} scan</strong></div><div class="fpp-value"><span>Total canvas</span><strong>${esc(f.display_size)}</strong></div><div class="fpp-value"><span>Panel output</span><strong>${f.output_ready?'Configured':'Needs applying'}</strong></div><div class="fpp-value"><span>Frame input</span><strong>${f.input_ready?'Ready':'Needs applying'}</strong></div>`;
      const eng=f.engineering||{};$('controllerOutputEngineering').innerHTML=`<div class="fpp-value"><span>Platform driver</span><strong>${esc(eng.matrix?.subType||'—')}</strong></div><div class="fpp-value"><span>Driver mapping</span><strong>${esc(eng.matrix?.wiringPinout||eng.matrix?.configName||eng.cape_panel_profile||'Platform-managed')}</strong></div><div class="fpp-value"><span>Channel range</span><strong>${f.start_channel} – ${(f.start_channel+f.channel_count-1).toLocaleString()}</strong></div><div class="fpp-value"><span>Channel count</span><strong>${f.channel_count.toLocaleString()}</strong></div><div class="fpp-value"><span>DDP frame input</span><strong>${esc(eng.ddp_destination||`127.0.0.1:${f.ddp_port}`)}</strong></div><div class="fpp-value"><span>Physical mapping</span><strong>${eng.used_existing_mapping?'Preserved from existing configuration':'Generated from current layout'}</strong></div>`;
      $('applyControllerOutput').disabled=f.can_apply===false;return f;
    }catch(e){$('fppSetup').innerHTML=`<p class="muted">${esc(e.message)}</p>`;$('controllerOutputMessage').textContent=e.message;$('controllerOutputMessage').classList.add('warn');throw e;}
  }
  function renderControllerHealth(h){
    const badge=$('controllerHealthBadge'),repair=$('repairControllerHealth'),list=$('controllerHealthList');
    if(badge){badge.textContent=h?.healthy&&!h?.drifted?'Healthy':h?.drifted?'Drift detected':'Attention';badge.className=`health-badge ${h?.healthy&&!h?.drifted?'ok':h?.drifted?'warn':'error'}`;}
    if(list)list.innerHTML=(h?.checks||[]).map(c=>`<div class="controller-health-item ${c.ok?'ok':'warn'}"><strong>${esc(c.label)} · ${c.ok?'OK':'Needs attention'}</strong><span>${esc(c.detail||'')}</span></div>`).join('')||'<div class="controller-health-item warn"><strong>Health unavailable</strong><span>No controller checks were returned.</span></div>';
    if(repair){const show=!!h?.repairable&&can('users');repair.classList.toggle('hidden',!show);repair.disabled=!show;}
    $('controllerOutputMessage').textContent=h?.message||'Controller health checked';$('controllerOutputMessage').classList.toggle('warn',!h?.healthy||!!h?.drifted);
  }
  async function loadControllerHealth(){try{const h=await api('/api/controller-health');renderControllerHealth(h);return h;}catch(e){renderControllerHealth({healthy:false,drifted:false,repairable:false,message:e.message,checks:[]});throw e;}}
  async function refreshControllerSection(){const values=await Promise.allSettled([loadInterfaceMode(),loadFppSetup(),loadControllerHealth(),loadControllerUpdate(false)]);return values;}
  async function repairControllerHealth(){
    if(!confirm('Repair the managed controller configuration? This restores Pi Matrix DDP to 127.0.0.1:4048 with channel offset 0, re-applies the selected controller interface mode and regenerates the panel-output configuration.'))return;
    $('repairControllerHealth').disabled=true;try{const h=await api('/api/controller-health/repair',{method:'POST',body:{}});state.settings=await api('/api/settings');populateSettings();renderControllerHealth(h);await loadFppSetup();toast(h.healthy?'Controller configuration repaired':'Repair completed but the controller still needs attention',!h.healthy);}catch(e){toast(e.message,true);await loadControllerHealth().catch(()=>{});}finally{$('repairControllerHealth').disabled=false;}
  }
  async function applyControllerOutputConfig({confirmFirst=true,quiet=false}={}){
    if(confirmFirst&&!confirm('Apply the current Display settings to the panel controller? Pi Matrix Signage will replace its managed LED-panel output and enable local frame input. Unrelated controller outputs are preserved.'))return null;
    $('applyControllerOutput').disabled=true;try{const r=await api('/api/controller-output/apply',{method:'POST',body:{}});await Promise.allSettled([loadFppSetup(),loadControllerHealth()]);if(!quiet)toast(r.message||'Panel controller configuration applied');return r;}catch(e){await Promise.allSettled([loadFppSetup(),loadControllerHealth()]);if(!quiet)toast(e.message,true);throw e;}finally{$('applyControllerOutput').disabled=false;}
  }
  function controllerUpdateBusy(data){const stateName=String(data?.status?.state||'').toLowerCase();return ['queued','checking','installing','restarting'].includes(stateName);}
  function formatUpdateChecked(value){if(!value)return 'Waiting…';const d=new Date(value);return Number.isNaN(d.getTime())?'Checked':d.toLocaleString();}
  function renderSoftwareUpdateNotice(data){
    const app=!!data?.available&&!!data?.platform_ready,plat=!!data?.controller_platform?.update_available,busy=controllerUpdateBusy(data),show=(app||plat)&&!busy,banner=$('softwareUpdateBanner'),badge=$('settingsUpdateBadge');
    if(banner){banner.classList.toggle('hidden',!show);if(show){const latest=String(data?.latest_version||'').trim(),target=String(data?.controller_platform?.certified_version||'').trim();let text='Software updates are ready to install.';if(app&&plat)text=`Pi Matrix Signage${latest?` v${latest}`:''} and certified controller platform FPP${target?` ${target}`:''} are available.`;else if(app)text=latest?`Pi Matrix Signage v${latest} is ready to install · currently v${data?.current_version||'—'}.`:'A newer Pi Matrix Signage release is ready to install.';else if(plat)text=`ISSL-certified controller platform FPP ${target||'update'} is ready to install.`;$('softwareUpdateBannerText').textContent=text;}}
    if(badge)badge.classList.toggle('hidden',!show);
  }
  function openControllerSoftware(){
    if(!can('display_setup'))return;document.querySelector('.tab[data-tab="setup"]')?.click();setSetupSubtab('controller',{persist:true,refresh:true});setTimeout(()=>$('controllerSoftwareCard')?.scrollIntoView({behavior:'smooth',block:'start'}),80);
  }
  function renderControllerUpdate(data){
    const st=data?.status||{},sharedBusy=controllerUpdateBusy(data),appKind=String(st.kind||'')==='pimatrix',appState=appKind?String(st.state||'').toLowerCase():'';
    $('controllerUpdateCurrent').textContent=`v${data?.current_version||'—'}`;$('controllerUpdateStatus').textContent=data?.available?(data?.latest_version?`v${data.latest_version} available`:'Update available'):data?.platform_ready?'Up to date':data?.checked_at?'Unable to check':'Waiting for background check';$('controllerUpdateInstallState').textContent=appState?appState.charAt(0).toUpperCase()+appState.slice(1):'Ready';$('controllerUpdateChecked').textContent=formatUpdateChecked(data?.checked_at);$('controllerUpdateMessage').textContent=appKind&&st.message?st.message:(data?.message||'Pi Matrix Signage software status');$('controllerUpdateMessage').classList.toggle('warn',!!data?.checked_at&&!data?.platform_ready||appState==='failed');const appAllowed=can('users')&&!!data?.helper_ready&&!!data?.available&&!sharedBusy;$('installControllerUpdate').classList.toggle('hidden',!appAllowed);$('installControllerUpdate').disabled=!appAllowed;
    const p=data?.controller_platform||{},pst=p.status||{},pstate=String(pst.state||'').toLowerCase(),pbusy=['queued','installing','restarting'].includes(pstate);$('platformInstalledVersion').textContent=p.installed_version&&p.installed_version!=='unknown'?`FPP ${p.installed_version}`:'Unknown';$('platformCertifiedVersion').textContent=p.certified_version?`FPP ${p.certified_version}`:'Not defined';$('platformCertificationStatus').textContent=p.certified?'Certified':p.certification_valid?(p.installed_version&&p.installed_version!=='unknown'?'Outside certified target':'Unable to verify'):'No matching certification';$('platformUpdateStatus').textContent=pbusy?(pstate.charAt(0).toUpperCase()+pstate.slice(1)):p.update_available?'Certified update available':p.certified?'Up to date':'No managed update';$('platformUpdateMessage').textContent=pbusy&&pst.message?pst.message:(p.message||'Controller platform status');$('platformUpdateMessage').classList.toggle('warn',!p.certified&& !p.update_available || pstate==='failed');const pAllowed=can('users')&&!!data?.helper_ready&&!!p.update_available&&!sharedBusy;$('installPlatformUpdate').classList.toggle('hidden',!pAllowed);$('installPlatformUpdate').disabled=!pAllowed;$('checkControllerUpdate').disabled=sharedBusy;renderSoftwareUpdateNotice(data);
  }
  async function loadControllerUpdate(forceCheck=false){try{const d=await api(`/api/controller-update${forceCheck?'?check=1':''}`);renderControllerUpdate(d);return d;}catch(e){$('controllerUpdateStatus').textContent='Unavailable';$('controllerUpdateMessage').textContent=e.message;$('controllerUpdateMessage').classList.add('warn');return null;}}
  async function installControllerUpdate(){
    if(!confirm('Install the available Pi Matrix Signage update now? The web interface may disappear briefly while the application restarts.'))return;try{const r=await api('/api/controller-update/install',{method:'POST',body:{}});toast(r.message||'Software update queued');$('controllerUpdateInstallState').textContent='Queued';$('installControllerUpdate').classList.add('hidden');let tries=0;const poll=async()=>{tries++;try{const d=await loadControllerUpdate(false);const st=String(d?.status?.state||'').toLowerCase(),kind=String(d?.status?.kind||'');if(kind==='pimatrix'&&st==='completed'){toast('Pi Matrix Signage update completed');setTimeout(()=>location.reload(),900);return;}if(kind==='pimatrix'&&st==='failed'){toast(d?.status?.message||'Software update failed',true);return;}}catch(_){/* expected while the app restarts */}if(tries<90)setTimeout(poll,2000);else location.reload();};setTimeout(poll,1800);}catch(e){toast(e.message,true);}
  }
  async function installPlatformUpdate(){
    const target=$('platformCertifiedVersion').textContent;if(!confirm(`Install the ${target} certified controller-platform update? A full Pi Matrix + FPP backup will be created first. The controller will reboot and can be unavailable for several minutes.`))return;
    $('installPlatformUpdate').disabled=true;try{const r=await api('/api/controller-platform-update/install',{method:'POST',body:{}});toast(r.message||'Controller platform update queued');$('platformUpdateMessage').textContent=`${r.message||'Update queued'} Pre-update backup: ${r.backup_filename||'created'}.`;let tries=0,seenOffline=false;const poll=async()=>{tries++;try{const hr=await fetch('/health',{cache:'no-store'});if(hr.ok){if(seenOffline||tries>3){const d=await loadControllerUpdate(true);const p=d?.controller_platform||{},ps=String(p?.status?.state||'').toLowerCase();if(p.certified||ps==='completed'){toast('Certified controller platform update completed');setTimeout(()=>location.reload(),1000);return;}if(ps==='failed'){toast(p?.status?.message||'Controller platform update failed',true);return;}}}else seenOffline=true;}catch(_){seenOffline=true;}if(tries<240)setTimeout(poll,5000);else{toast('Controller platform update is taking longer than expected. Check System diagnostics when the controller is reachable.',true);}};setTimeout(poll,5000);}catch(e){toast(e.message,true);$('installPlatformUpdate').disabled=false;}
  }

  async function openEngineeringAccess(){
    const password=$('engineeringPassword').value;if(!password){toast('Enter your current password first',true);return;}const win=window.open('','_blank');try{const r=await api('/api/engineering-access',{method:'POST',body:{password}});$('engineeringPassword').value='';if(win)win.location=r.url;else window.open(r.url,'_blank');toast('Engineering interface opened in a new tab');}catch(e){if(win)win.close();toast(e.message,true);}
  }

  function renderCommissioning(){const s=state.settings||{},passed=!!s.colorlight_commissioned;$('commissioningBadge').textContent=passed?'Commissioned':'Not commissioned';$('commissioningBadge').className=`health-badge ${passed?'ok':'warn'}`;$('commissioningDetail').textContent=passed?`Passed ${new Date(s.colorlight_commissioned_at).toLocaleString()} by ${s.colorlight_commissioned_by||'operator'}`:'Complete all tests to record commissioning.';}
  function updateDetectedInterfaceWarning(){const chosen=(state.colorlightInterfaces||[]).find(i=>i.name===$('detectedColorlightInterface').value);$('interfaceWarning').textContent=!chosen?'No Ethernet interfaces detected.':chosen.is_default?'Warning: this interface carries the normal LAN/default route. Choose a separate adapter for Colorlight.':`${chosen.name} is available and is not the default-route interface.`;$('interfaceWarning').classList.toggle('warn',!!chosen?.is_default);}
  ...
  function renderHardwareProfiles(){
    ...
    const labels={rpi_mfc:'Hanson rPI-MFC',colorlight:'Colorlight',adafruit_hat:'Adafruit RGB Matrix HAT / Bonnet',adafruit_triple:'Adafruit Triple Matrix Bonnet'};
    const row=p=>{const c=p.config||{},badge=p.builtin?'<span class="backup-pill">Built-in starter</span>':'<span class="backup-pill safety">Saved</span>',remove=p.builtin?'':`<button class="btn danger ghost small" data-profile-delete="${p.id}">Delete</button>`,hardware=c.panel_output_type==='colorlight'?`Colorlight ${(c.colorlight_receiver_model||'').toUpperCase()} · ${c.colorlight_interface}`:(labels[c.panel_output_type]||c.panel_output_type||'Unknown output'),canvas=`${(+c.panel_width||0)*(+c.panels_across||0)}×${(+c.panel_height||0)*(+c.panels_down||0)}`;return `<div class="backup-row"><div class="backup-row-main"><div><strong>${esc(p.name)}</strong>${badge}</div><small>${esc(hardware)} · panel ${c.panel_width}×${c.panel_height} · ${esc(c.panel_scan)} · ${c.panels_across}×${c.panels_down} panels · canvas ${canvas} · ${c.brightness}% max</small>${p.builtin?'<small class="muted">Sample only — verify scan rate, driver mapping and colour order against the panel; also confirm panel orientation before commissioning.</small>':''}</div><div class="backup-row-actions"><button class="btn secondary small" data-profile-apply="${p.id}">Apply</button>${remove}</div></div>`;};
    const order=['rpi_mfc','colorlight','adafruit_hat','adafruit_triple'];
    const samples=profiles.filter(p=>p.builtin),saved=profiles.filter(p=>!p.builtin);
    let out='<div class="profile-summary"><strong>Built-in samples</strong><span>Choose the closest layout, apply it, then adjust and save your commissioned setup.</span></div>';
    for(const type of order){const group=samples.filter(p=>(p.config||{}).panel_output_type===type);if(group.length)out+=`<div class="profile-group"><h3>${esc(labels[type]||type)} <span>${group.length} sample${group.length===1?'':'s'}</span></h3>${group.map(row).join('')}</div>`;}
    if(saved.length)out+=`<div class="profile-group saved-profile-group"><h3>Your saved profiles <span>${saved.length}</span></h3>${saved.map(row).join('')}</div>`;
    else out+='<div class="profile-group saved-profile-group"><h3>Your saved profiles</h3><p class="muted">No site-specific profiles saved yet. Configure and commission a display, enter a profile name above, then choose Save current.</p></div>';
    list.innerHTML=out;
    $$('[data-profile-apply]').forEach(b=>b.addEventListener('click',()=>applyHardwareProfile(+b.dataset.profileApply)));$$('[data-profile-delete]').forEach(b=>b.addEventListener('click',()=>deleteHardwareProfile(+b.dataset.profileDelete)));
  }
  async function saveHardwareProfile(){const name=$('hardwareProfileName').value.trim();if(!name){toast('Enter a profile name',true);return;}try{await api('/api/hardware-profiles',{method:'POST',body:{name}});$('hardwareProfileName').value='';await loadHardwareProfiles();toast('Hardware profile saved');}catch(e){toast(e.message,true);}}
  async function applyHardwareProfile(id){if(!confirm('Apply this hardware profile? Current display hardware settings will be replaced, the panel controller will be reconfigured and commissioning will be cleared.'))return;try{state.settings=await api(`/api/hardware-profiles/${id}/apply`,{method:'POST',body:{}});populateSettings();let applied=false;try{await applyControllerOutputConfig({confirmFirst:false,quiet:true});applied=true;}catch(_){await loadFppSetup().catch(()=>{});}await Promise.allSettled([detectColorlightInterfaces(true),loadControllerHealth()]);toast(applied?'Hardware profile and panel controller applied':'Hardware profile applied; panel controller needs attention',!applied);}catch(e){toast(e.message,true);}}
  async function deleteHardwareProfile(id){if(!confirm('Delete this hardware profile?'))return;try{await api(`/api/hardware-profiles/${id}`,{method:'DELETE'});await loadHardwareProfiles();toast('Hardware profile deleted');}catch(e){toast(e.message,true);}}
  function createSupportPackage(){const preview=$('supportIncludePreview').checked?'1':'0';location.href=`/api/support-package?include_preview=${preview}`;toast('Creating support package…');}
  async function factoryResetController(){
    const password=$('factoryResetPassword').value,confirmation=$('factoryResetConfirmation').value.trim(),acknowledged=$('factoryResetAcknowledge').checked,clearNetwork=$('factoryResetClearNetwork').checked;
    if(!password){toast('Enter your current password',true);return;}
    if(confirmation!=='RESET THIS CONTROLLER'){toast('Type RESET THIS CONTROLLER exactly',true);return;}
    if(!acknowledged){toast('Confirm that you understand the reset is permanent',true);return;}
    const networkText=clearNetwork?'Controller network/Wi-Fi settings will also be erased, this browser connection will be lost, and the Pi will reboot.':'The current controller network configuration will be retained.';
    if(!confirm(`Factory reset this controller?\n\n${networkText}\n\nAll Pi Matrix Signage customer data, users, media, local licence data, backups and underlying controller site/output configuration will be permanently erased.`))return;
    const button=$('factoryResetController');button.disabled=true;button.textContent='Starting factory reset…';
    try{const r=await api('/api/factory-reset',{method:'POST',body:{password,confirmation,acknowledged:true,clear_network:clearNetwork}});alert(`${r.message}\n\nWhen the reset completes, Pi Matrix Signage starts with admin / pimatrix and requires an immediate password change.`);if(!clearNetwork)setTimeout(()=>{location.href='/login';},2500);}
    catch(e){button.disabled=false;button.textContent='Factory reset controller…';toast(e.message,true);}
  }
  ...
  function renderDiagnostics(d,syncControls=false){
    ...
    $('diagFppd').textContent=String(svc.fppd||'').toLowerCase()==='active'?'Running':String(svc.fppd||'').toLowerCase()==='running'?'Running':String(svc.fppd||'unknown').toLowerCase()==='inactive'?'Stopped':String(svc.fppd||'unknown');$('diagDdp').textContent=svc.ddp_listening?`DDP UDP ${svc.ddp_port} listening`:`No DDP listener on UDP ${svc.ddp_port}`;
  ...
  function nextScheduleCountdown(startAt){
    const seconds=Math.max(0,Math.round((new Date(startAt).getTime()-Date.now())/1000));
    if(seconds<60)return 'Starts in less than a minute';
    if(seconds<3600)return `Starts in ${Math.ceil(seconds/60)} min`;
    if(seconds<86400){const h=Math.floor(seconds/3600),m=Math.floor((seconds%3600)/60);return `Starts in ${h}h${m?` ${m}m`:''}`;}
    const d=Math.floor(seconds/86400),h=Math.floor((seconds%86400)/3600);return `Starts in ${d}d${h?` ${h}h`:''}`;
  }
  function renderNextSchedule(item){
    const summary=$('nextScheduleSummary'),empty=$('nextScheduleEmpty');if(!summary||!empty)return;
    const has=!!item;summary.classList.toggle('hidden',!has);empty.classList.toggle('hidden',has);if(!has)return;
    $('nextScheduleName').textContent=item.name||'Schedule';
    $('nextScheduleContent').textContent=`${item.target_type==='playlist'?'Playlist':'Message'} · ${item.target_name||('#'+item.target_id)}`;
    $('nextScheduleStarts').textContent=item.start_label||'—';
    $('nextScheduleWindow').textContent=item.window_label||'—';
    $('nextScheduleCountdown').textContent=nextScheduleCountdown(item.start_at);
    $('nextScheduleContent').classList.toggle('error-text',item.target_available===false);
  }

  async function refreshStatus(){try{const s=await api('/api/status');const p=$('statusPill');p.classList.toggle('ok',s.running&&!s.last_error&&!s.emergency);p.classList.toggle('error',!!s.last_error||!!s.emergency);p.querySelector('span:last-child').textContent=s.emergency?'EMERGENCY':s.last_error?'Output error':'Running';state.displayWidth=+s.width||0;state.displayHeight=+s.height||0;$('displaySize').textContent=`${s.width} × ${s.height} pixels`;$('framesSent').textContent=Number(s.frames_sent).toLocaleString();$('lastError').textContent=s.last_error||'None';$('activeSource').textContent=s.transition?`Exiting message #${s.transition.outgoing_message_id} · ${Number(s.transition.remaining||0).toFixed(1)}s`:s.emergency?`EMERGENCY · message #${s.emergency.id}`:s.manual?`Manual ${s.manual.type}${s.manual.id?' #'+s.manual.id:''}`:s.active?`${s.active.source} · ${s.active.type} #${s.active.id}`:'Blank';$('outputTarget').textContent=state.settings?`${state.settings.ddp_host}:${state.settings.ddp_port} · ${s.brightness?.effective??state.settings.brightness}%`:'DDP';renderNextSchedule(s.next_schedule);renderSoftwareUpdateNotice(s.software_update);if($('emergencyPanel'))$('emergencyPanel').classList.toggle('active',!!s.emergency);$('activateEmergency').disabled=!!s.emergency||!state.settings?.emergency_message_id;$('clearEmergency').disabled=!s.emergency;if(can('schedules')&&$('page-schedules').classList.contains('active')&&Date.now()-(state._lastRuleRefresh||0)>4000){state._lastRuleRefresh=Date.now();api('/api/conditional-rules').then(v=>{state.conditionalRules=v;renderConditionalRuleList();if(state.selectedConditionalRule)selectConditionalRule(state.selectedConditionalRule);}).catch(()=>{});}}catch(e){const p=$('statusPill');p.classList.add('error');p.querySelector('span:last-child').textContent='Disconnected';}}
  ...
  $('addCloudTextLayer').addEventListener('click',()=>addLayer('cloud-text'));
  ...
  $('openSchedulesFromDashboard').addEventListener('click',()=>document.querySelector('.tab[data-tab="schedules"]')?.click());$('viewSoftwareUpdate').addEventListener('click',openControllerSoftware);
  ...
  $('panelOutputType').addEventListener('change',updateOutputHardware);
  $('detectInterfaces').addEventListener('click',()=>detectColorlightInterfaces());$('useDetectedInterface').addEventListener('click',()=>{$('colorlightInterface').value=$('detectedColorlightInterface').value;toast('Interface selected; save Display settings to apply it');});$('testColorlightFpp').addEventListener('click',testColorlightFpp);$$('.wizard-pattern').forEach(b=>b.addEventListener('click',async()=>{try{await api('/api/test-pattern',{method:'POST',body:{kind:b.dataset.pattern,duration:30}});toast(`${b.textContent} test running for 30 seconds`);}catch(e){toast(e.message,true);}}));$('completeCommissioning').addEventListener('click',completeColorlightCommissioning);$('saveHardwareProfile').addEventListener('click',saveHardwareProfile);$('createSupportPackage').addEventListener('click',createSupportPackage);
  $('detectedColorlightInterface').addEventListener('change',updateDetectedInterfaceWarning);
    ['panelWidth','panelHeight','panelsAcross','panelsDown'].forEach(id=>$(id).addEventListener('input',updateLayoutSummary));$('brightness').addEventListener('input',()=>{$('brightnessValue').textContent=`${$('brightness').value}%`;});$('saveSettings').addEventListener('click',saveSettings);$('activateLicence').addEventListener('click',activateLicence);$('checkLicence').addEventListener('click',checkLicence);$('clearLocalLicence').addEventListener('click',clearLocalLicence);$('openLicenseSetup').addEventListener('click',openLicenceSetup);$('saveInterfaceMode').addEventListener('click',saveInterfaceMode);$('openFppInterface').addEventListener('click',openFppInterface);$('refreshFpp').addEventListener('click',refreshControllerSection);$('repairControllerHealth').addEventListener('click',repairControllerHealth);$('applyControllerOutput').addEventListener('click',()=>applyControllerOutputConfig());$('checkControllerUpdate').addEventListener('click',()=>loadControllerUpdate(true));$('installControllerUpdate').addEventListener('click',installControllerUpdate);$('installPlatformUpdate').addEventListener('click',installPlatformUpdate);$('openEngineeringAccess').addEventListener('click',openEngineeringAccess);$('saveGpioControls').addEventListener('click',saveGpioControls);$$('[data-gpio-field="action"]').forEach(x=>x.addEventListener('change',syncGpioEmergencyFields));$$('[data-gpio-test]').forEach(x=>x.addEventListener('click',()=>testGpioAction(x.dataset.gpioTest)));$('shutdownPi').addEventListener('click',shutdownPi);$('refreshDiagnostics').addEventListener('click',()=>loadDiagnostics(true));$('saveRecoverySettings').addEventListener('click',saveRecoverySettings);$('restartRenderer').addEventListener('click',()=>diagnosticsAction('restart-renderer','Restart the LED renderer now? The web interface stays online.'));$('restartFppd').addEventListener('click',()=>diagnosticsAction('restart-fppd','Restart the panel output service now? The LED output may pause briefly.'));$('clearRecoveryHistory').addEventListener('click',()=>diagnosticsAction('clear-history','Clear the recovery history?'));

  $('createBackup').addEventListener('click',createBackup);$('refreshBackups').addEventListener('click',()=>loadBackups());$('factoryResetController').addEventListener('click',factoryResetController);$('exportConfiguration').addEventListener('click',()=>downloadPortable('configuration'));$('importConfigurationFile').addEventListener('change',async()=>{const f=$('importConfigurationFile').files?.[0];if(f){const r=await importPortableFile(f,'configuration');if(r){await loadAll();toast('Portable configuration imported');}}$('importConfigurationFile').value='';});
