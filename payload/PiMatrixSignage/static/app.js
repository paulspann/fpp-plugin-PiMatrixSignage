(() => {
  ...
  function renderHardwareDetection(){
    ...
    if(status){
      if(detected){
        ...
        status.innerHTML=`<strong>Hanson rPi-MFC detected:</strong> ${esc(d.cape_name||'Hanson rPi-MFC')}${d.cape_version?` · version ${esc(d.cape_version)}`:''}. ${esc(d.message||'FPP has confirmed the Hanson cape hardware.')} Hanson, Colorlight and the two Adafruit adapter choices are available.`;
      }else{
        status.classList.add('warn');
        status.innerHTML=d.source==='virtual_rpi_mfc_only'
          ?'<strong>rPi-MFC configured but not physically confirmed:</strong> FPP has an rPi-MFC virtual cape identity, but no physical Hanson EEPROM was confirmed. Hanson is hidden. Colorlight and Adafruit adapters remain available; Adafruit boards do not provide a reliable common auto-detection signal and must be selected manually.'
  ...
  function updateOutputHardware(){
    ...
    if(adafruitHelp&&adafruitHat){
      adafruitHelp.innerHTML='<strong>FPP direct-HUB75 setup:</strong> in FPP LED Panels use RGBMatrix with the <code>adafruit-hat</code> wiring pinout and one parallel output. Do not choose <code>adafruit-hat-pwm</code> unless the documented GPIO4→GPIO18 hardware modification is physically present. Current FPP RGBMatrix direct output is for Raspberry Pi 4-class controllers, not Pi 5.';
    }else if(adafruitHelp&&adafruitTriple){
      adafruitHelp.innerHTML='<strong>FPP Active3 setup:</strong> in FPP LED Panels use RGBMatrix with the <code>regular</code> wiring pinout and <strong>3 parallel outputs</strong>. Each IDC socket is a separate parallel HUB75 string. Provide panel power separately; the Triple Matrix Bonnet does not power the LED panels. Current FPP RGBMatrix direct output is for Raspberry Pi 4-class controllers, not Pi 5.';
    }
    ...
    if(colorlight){
      $('panelHardwareWarning').innerHTML='<strong>Colorlight mapping:</strong> configure panel driver IC, scan rate and cabinet mapping on the receiver card first. Then create the matching ColorLight 5A-75 output in FPP.';
    }else if(adafruitHat){
      $('panelHardwareWarning').innerHTML='<strong>Adafruit HAT / Bonnet:</strong> configure FPP LED Panels for RGBMatrix using the <code>adafruit-hat</code> pinout. This FPP path supports one parallel output; verify the panel scan/address mapping and any Address-E jumper required by 64×64 panels.';
    }else if(adafruitTriple){
      $('panelHardwareWarning').innerHTML='<strong>Adafruit Triple / Active3:</strong> configure FPP LED Panels for RGBMatrix using the <code>regular</code> pinout with 3 parallel outputs. Map each physical string to the required logical layout and power the panels independently.';
    }else{
      $('panelHardwareWarning').innerHTML='<strong>Physical panel order/orientation:</strong> set each panel wiring order and rotation once in FPP LED Panels. The rPI-MFC maximum panel count depends on panel size and scan rate.';
    }
  }
  ...
  async function applyHardwareProfile(id){if(!confirm('Apply this hardware profile? Current display hardware settings will be replaced and commissioning will be cleared.'))return;try{state.settings=await api(`/api/hardware-profiles/${id}/apply`,{method:'POST',body:{}});populateSettings();await Promise.all([loadFppSetup(),detectColorlightInterfaces(true)]);toast('Hardware profile applied');}catch(e){toast(e.message,true);}}
  ...
  async function factoryResetController(){
    ...
    const networkText=clearNetwork?'FPP network/Wi-Fi settings will also be erased, this browser connection will be lost, and the Pi will reboot.':'The current FPP network configuration will be retained.';
    if(!confirm(`Factory reset this controller?\n\n${networkText}\n\nAll Pi Matrix Signage customer data, users, media, local licence data, backups and FPP site/output configuration will be permanently erased.`))return;
